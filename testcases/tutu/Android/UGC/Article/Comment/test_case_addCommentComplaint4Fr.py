#!/usr/bin/python3
# -*- coding: utf-8 -*-
# @Time    : 2020/2/28 11:07
# @Author  : Durat
# @Email   : durant.zeng@sunvalley.com.cn
# @File    : test_case_addCommentComplaint4Fr.py
# @Software: PyCharm
...
from interface.tutu.UGC.Article.Comment.addCommentComplaint4Fr import addCommentComplaint4Fr
from common.excelUtil import excelUtil


class TestaddCommentComplaint4FrFunc(unittest.TestCase):
    """Test addCommentComplaint4Fr.py"""
    ...
    def setUp(self):
        ...
        self.addCommentComplaint = addCommentComplaint4Fr()
    ...
    def test_addCommentComplaint4Fr_tutu_Android_001(self):
        '''美甲涂涂移动Android端_addCommentComplaint4Fr_投诉-对评论的投诉_001'''
        ...
        try:
            ...
            # #addCommentComplaint4Fr接口
            addCommentComplaintURL = self.addCommentComplaint.get_addCommentComplaint4FrURL(self.config.get('imi_base_url_ch', 'base_url_dev_k8s'),
                                                      self.config.get("UGC", "addCommentComplaint4FrURL"), self.config.get("lang", "zh"),
                                                      self.base.getTimeStamp(),
                                                     self.config.get("clientVersionInfo", "clientVersionInfo_ch_Android"),access_token)

            articleId = result_getCommentList["data"][0]["articleId"]
            commentId = result_getCommentList["data"][0]["id"]
            replyType = 2
            content = result_getCommentList["data"][0]["content"]
            uid = result_getCommentList["data"][0]["uid"]
            complaintType = 2

            time.sleep(0.5)
            result_addCommentComplaint = self.addCommentComplaint.send_request_addCommentComplaint4Fr(addCommentComplaintURL,articleId,commentId,replyType,content,uid,complaintType)
            self.assertEqual(result_addCommentComplaint["stateCode"], 200)
            self.assertEqual(result_addCommentComplaint["stateMsg"], "OK")
