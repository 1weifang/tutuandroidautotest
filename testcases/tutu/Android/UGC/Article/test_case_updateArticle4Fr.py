#!/usr/bin/python3
# -*- coding: utf-8 -*-
# @Time    : 2020/2/27 14:28
# @Author  : Durat
# @Email   : durant.zeng@sunvalley.com.cn
# @File    : test_case_updateArticle4Fr.py
# @Software: PyCharm
...
class TestupdateArticle4FrFunc(unittest.TestCase):
    """Test updateArticle4Fr.py"""
    ...
    def test_updateArticle4Fr_tutu_Android_001(self):
        '''美甲涂涂移动Android端_updateArticle4Fr_UGC修改笔记_图片_001'''
        #账号密码登录
        ...
        try:
            ...
            time.sleep(0.5)
            #updateArticle4Fr接口
            updateArticle4URL = self.updateArticle.get_updateArticle4FrURL(self.config.get('imi_base_url_ch', 'base_url_dev_k8s'),
                                                      self.config.get("UGC", "updateArticle4FrURL"), self.config.get("lang", "zh"),
                                                      self.base.getTimeStamp(),

                                                     self.config.get("clientVersionInfo", "clientVersionInfo_ch_Android"),access_token)

            content1 = self.base.get_random_content()

            result_addArticle = self.updateArticle.send_request_updateArticle4Fr(updateArticle4URL,id,articleType,articleStatus,imagesFallsUrl,imagesFallsWidth,imagesFallsHeight,imagesFallsLitimgUrl,imagesUrlsList,imagesLitimgUrlsList,content1)
            self.assertEqual(result_addArticle["stateCode"], 200)
            self.assertEqual(result_addArticle["stateMsg"], "OK")
