#!/usr/bin/python3
# -*- coding: utf-8 -*-
# @Time    : 2020/2/18 14:16
# @Author  : Durat
# @Email   : durant.zeng@sunvalley.com.cn
# @File    : test_case_staticUserLog4Fr.py
# @Software: PyCharm
...
class TeststaticUserLog4FrFunc(unittest.TestCase):
    """Test staticUserLog4Fr.py"""
    ...
    def test_staticUserLog4Fr_tutu_Android_001(self):
        '''美甲涂涂移动Android端_staticUserLog4Fr_ugc文章-点赞_001'''
        #账号密码登录
        ...
        try:
            ...
            time.sleep(0.5)
            #ugc笔记-统计点赞/转发
            staticURL = self.static.get_staticUserLog4FrURL(self.config.get('imi_base_url_ch', 'base_url_dev_k8s'),
                                                      self.config.get("UGC", "staticUserLog4FrURL"), self.config.get("lang", "zh"),
                                                      self.base.getTimeStamp(),
                                                      self.config.get("clientVersionInfo", "clientVersionInfo_ch_Android"),access_token)

            staticType = 2
            result_static = self.static.send_request_staticUserLog4Fr(staticURL,staticType,id)
    ...
    def test_staticUserLog4Fr_tutu_Android_002(self):
        '''美甲涂涂移动Android端_staticUserLog4Fr_ugc文章-微信转发_002'''
        #账号密码登录
        TestData = self.ex.getDict(2, 29, 7, self.testData)
        currentPage = TestData["currentPage"]
        pageSize = TestData["pageSize"]
        ...
        try:
            ...
            staticType = 3
            shareWay = 1
            result_static = self.static.send_request_staticUserLog4Fr(staticURL,staticType,id,shareWay)
            self.assertEqual(result_static["stateCode"], 200)
            self.assertEqual(result_static["stateMsg"], "OK")
    ...
    def test_staticUserLog4Fr_tutu_Android_003(self):
        '''美甲涂涂移动Android端_staticUserLog4Fr_ugc文章-微博转发_003'''
        #账号密码登录
        TestData = self.ex.getDict(2, 29, 7, self.testData)
        currentPage = TestData["currentPage"]
        pageSize = TestData["pageSize"]
    ...
    def test_staticUserLog4Fr_tutu_Android_004(self):
        '''美甲涂涂移动Android端_staticUserLog4Fr_ugc文章-QQ转发_004'''
        #账号密码登录
        TestData = self.ex.getDict(2, 29, 7, self.testData)
        currentPage = TestData["currentPage"]
        pageSize = TestData["pageSize"]
